...
export async function post({ request }) {
  ...
  try { 
    ...
    return new Response(JSON.stringify({ message: 'Invalid JSON' }), { status: 400, headers: { ...base, 'Content-Type': 'application/json' } }); 
  }

  // Ensure id, date, and status are present
  const { id, date, period, status, time, name } = body || {};
  if (!id || !date || !status) {
    return new Response(JSON.stringify({ message: 'Missing fields' }), { status: 400, headers: { ...base, 'Content-Type': 'application/json' } });
  }

  try {
    // markEdited: true ensures this manual edit is protected from future automated imports
